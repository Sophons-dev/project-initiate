        ...
        <Card className='h-full rounded border hover:shadow transition-all duration-300 flex flex-col'>
          <CardContent className='flex-1 flex flex-col'>
            <div className='flex-1'>
              ...
              {/* Title */}
              <h2 className='text-base font-semibold text-gray-900 mb-3'>{recommendation.opportunity?.title}</h2>

              {/* Organization & Location */}
              <div className='space-y-2 mb-4'>
                <div className='flex items-center text-sm text-gray-600 min-w-0'>
                  <Building2 className='w-4 h-4 mr-2 flex-shrink-0' />
                  <span className='truncate'>{recommendation.opportunity?.organization?.name}</span>
                </div>
                <div className='flex items-center text-sm text-gray-600 min-w-0'>
                  <MapPin className='w-4 h-4 mr-2 flex-shrink-0' />
                  <span className='truncate'>{recommendation.opportunity?.location}</span>
                </div>
              </div>

              {/* Description */}
              <p className='text-sm text-gray-600 mb-4 line-clamp-2'>{recommendation.opportunity?.description}</p>
                ...
                <div className='bg-yellow-50 border-l-3 rounded-l-none border-l-yellow-500 rounded-lg p-3 mb-4'>
                  <div className='flex items-start'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-yellow-700 leading-normal'>{recommendation.reasoning}</p>
                    </div>
                  </div>
                </div>
              )}
