        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div className="space-y-4 mb-8">
                <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${wantsAdvanced === true ? "border-cyan-500 bg-cyan-50" : "border-gray-200 hover:border-gray-300 bg-white"
                        }`}
                    onClick={() => setWantsAdvanced(true)}
                >
                    <div className="flex items-center space-x-3">
                        <div
                            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${wantsAdvanced === true ? "border-cyan-500 bg-cyan-500" : "border-gray-300"
                                }`}
                        >
                ...
                <motion.div
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${wantsAdvanced === false ? "border-cyan-500 bg-cyan-50" : "border-gray-200 hover:border-gray-300 bg-white"
                        }`}
                    onClick={() => setWantsAdvanced(false)}
                >
                    <div className="flex items-center space-x-3">
                        <div
                            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${wantsAdvanced === false ? "border-cyan-500 bg-cyan-500" : "border-gray-300"
                                }`}
                        >
